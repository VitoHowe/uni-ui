...
const pkgExam_utils_imageParser = require("../../utils/imageParser.js");
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const practiceSummary = common_vendor.ref({
      answered_count: 0,
      correct_count: 0,
      wrong_count: 0,
      accuracy: 0
    ...
    const showPracticeSummary = common_vendor.computed(() => isRandomMode.value);
    const getAnswerKey = () => {
    ...
    const progressPercent = common_vendor.computed(() => {
      ...
      if (isSpecialMode.value || isRealMode.value || practiceMode.value === "chapter") {
        return totalQuestions.value > 0 ? Math.round(currentQuestionNumber.value / totalQuestions.value * 100) : 0;
      }
    ...
    const parsedContent = common_vendor.computed(() => {
      ...
      return pkgExam_utils_imageParser.parseQuestionImages(currentQuestion.value.content, imageBankId.value, baseUrl);
    });
    const parsedExplanation = common_vendor.computed(() => {
      if (!currentQuestion.value || !currentQuestion.value.explanation)
        return "";
      const baseUrl = utils_constants.API_CONFIG.BASE_URL.replace("/api", "");
      return pkgExam_utils_imageParser.parseQuestionImages(currentQuestion.value.explanation, imageBankId.value, baseUrl);
    });
    const currentImageUrls = common_vendor.computed(() => {
      if (!currentQuestion.value)
        return [];
      const baseUrl = utils_constants.API_CONFIG.BASE_URL.replace("/api", "");
      return pkgExam_utils_imageParser.extractAllQuestionImages(currentQuestion.value, imageBankId.value, baseUrl);
    });
    common_vendor.onMounted(async () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:433", "获取胶囊按钮信息失败:", e);
      }
    ...
    const initExam = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:517", "📖 开始初始化考试，参数:", {
          bankId: bankId.value,
        ...
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:526", "✅ 题库信息:", bankData);
        bankInfo.value = {
        ...
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:535", "✅ 章节列表:", chaptersData);
        ...
        if (startChapterId.value) {
          ...
          common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:549", `📍 找到起始章节，索引: ${currentChapterIndex.value}`);
        } else {
          currentChapterIndex.value = 0;
          common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:552", "📍 使用第一个章节");
        }
        currentChapter.value = chapters.value[currentChapterIndex.value];
        currentQuestionNumber.value = startQuestionNumber.value;
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:558", "📍 当前章节:", currentChapter.value);
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:559", "📍 起始题号:", currentQuestionNumber.value);
        await loadQuestion();
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:564", `✅ 初始化完成，开始${practiceMode.value === "chapter" ? "章节" : "整卷"}练习`);
      } catch (error) {
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:567", "❌ 初始化失败:", error);
        ...
        loading.value = false;
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:575", "✅ 初始化loading状态已重置");
      }
    };
    const initSpecialExam = async () => {
      common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:580", "📘 初始化专项训练", {
    ...
    const initRandomExam = async () => {
      common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:602", "📘 初始化随机练习", { subjectId: subjectId.value });
      chapters.value = [];
      currentChapterIndex.value = 0;
    ...
    const initRealExam = async () => {
      common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:635", "📘 初始化真题练习", { paperId: paperId.value });
      chapters.value = [];
      currentChapterIndex.value = 0;
      currentChapter.value = null;
      bankInfo.value = {
    ...
    const loadQuestion = async () => {
      ...
      if (!currentChapter.value) {
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:664", "❌ currentChapter is null");
        return;
      }
      loading.value = true;
      try {
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:670", `📖 开始加载题目: 题库${bankId.value}, 章节${currentChapter.value.id}, 题号${currentQuestionNumber.value}`);
        const response = await utils_request.get(
        ...
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:678", "📡 题目数据响应:", response);
        if (response && response.question) {
          ...
          common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:704", `✅ 题目加载成功: ${currentChapter.value.chapter_name} 第${currentQuestionNumber.value}题`);
        } else {
          common_vendor.index.__f__("warn", "at pkg-exam/pages/exam/exam.vue:706", "⚠️ 响应中没有question字段:", response);
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:716", "❌ 加载题目失败:", error);
        common_vendor.index.showToast({
          title: error.message || "加载失败",
          icon: "none"
        });
        ...
        loading.value = false;
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:723", "✅ loading状态已重置为false");
      }
    };
    const loadSpecialQuestion = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:749", "❌ 加载专项题目失败:", error);
        common_vendor.index.showToast({
          title: error.message || "加载失败",
          icon: "none"
        });
    ...
    const loadRealQuestion = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:801", "❌ 加载真题失败:", error);
        common_vendor.index.showToast({
          title: error.message || "加载失败",
          icon: "none"
        });
...
是否继续下一章节？`,
    ...
    const saveProgress = async () => {
      ...
      try {
        if (practiceMode.value === "chapter") {
          ...
          common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:1052", "💾 章节进度已保存:", {
            mode: "chapter",
          ...
          common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:1079", "💾 整卷进度已保存:", {
            mode: "full",
            chapter: currentChapter.value.chapter_name,
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1088", "保存进度失败:", error);
      }
    };
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              ...
              common_vendor.index.__f__("log", "at pkg-exam/pages/exam/exam.vue:1146", "🔄 学习进度已重置");
            } catch (error) {
              common_vendor.index.hideLoading();
              common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1149", "重置进度失败:", error);
              common_vendor.index.showToast({
    ...
    const saveSpecialProgress = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1174", "❌ 保存专项进度失败:", error);
      }
    };
    ...
    const buildAttemptQuestionIds = () => {
      const ids = Object.keys(userAnswers.value).map((key) => {
        var _a;
        return (_a = questionCache.value[key]) == null ? void 0 : _a.id;
    ...
    const getPracticeSourceId = () => {
      ...
      if (isSpecialMode.value)
        return subjectChapterId.value;
      if (practiceMode.value === "chapter")
        return ((_a = currentChapter.value) == null ? void 0 : _a.id) || 0;
      return bankId.value;
    };
    const fetchPracticeSummary = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1237", "❌ 获取随机练习统计失败:", error);
      }
    };
    const submitPracticeAttempt = async () => {
      ...
      try {
        ...
        await utils_request.post("/practice/attempts", payload, { showLoading: false });
        attemptSubmitted.value = true;
      } catch (error) {
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1268", "❌ 提交练习统计失败:", error);
      }
    };
    const submitRealAttempt = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam/exam.vue:1285", "❌ 提交真题结果失败:", error);
        common_vendor.index.showToast({
    ...
    const goToWrongQuestions = () => {
      common_vendor.index.navigateTo({
        url: `/pkg-exam/pages/real-exam-wrong/real-exam-wrong?paperId=${paperId.value}&paperName=${encodeURIComponent(paperName.value)}`
      });
    };
...
正确率：${accuracy.value}%`;
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        n: showPracticeSummary.value
      }, showPracticeSummary.value ? {
        o: common_vendor.t(practiceSummary.value.answered_count),
        p: common_vendor.t(practiceSummary.value.accuracy),
        q: common_vendor.t(practiceSummary.value.wrong_count)
      } : {}, {
        r: loading.value
      }, loading.value ? {
        s: common_vendor.p({
        ...
        v: common_vendor.t(getTypeLabel(currentQuestion.value.type)),
        w: common_vendor.n("type-" + currentQuestion.value.type),
        x: common_vendor.f(3, (i, k0, i0) => {
          return {
            a: i,
            b: "7ab7c2ad-3-" + i0,
            c: common_vendor.p({
        ...
        y: common_vendor.t(currentQuestionNumber.value),
        z: parsedContent.value,
        A: common_vendor.o(handleImageClick),
        B: currentQuestion.value.tags && currentQuestion.value.tags.length > 0
      }, currentQuestion.value.tags && currentQuestion.value.tags.length > 0 ? {
        C: common_vendor.f(currentQuestion.value.tags, (tag, index, i0) => {
          return {
        ...
        D: common_vendor.f(currentQuestion.value.options, (option, index, i0) => {
          return common_vendor.e({
        ...
        H: common_vendor.t(formatAnswer(currentQuestion.value.answer)),
        I: common_vendor.t(userAnswer.value || "未作答"),
        J: common_vendor.n(isAnswerCorrect.value ? "correct" : "wrong"),
        K: currentQuestion.value.explanation
      }, currentQuestion.value.explanation ? {
        L: parsedExplanation.value
      } : {}, {
        M: common_vendor.o(handleImageClick)
      }) : {}, {
        N: common_vendor.p({
          type: "back",
          size: "16",
        ...
        O: !hasPrevQuestion.value && (practiceMode.value === "chapter" || !canSwitchToPrevChapter()),
        P: common_vendor.o(prevQuestion),
        Q: !showAnswer.value
      }, !showAnswer.value ? {
        R: common_vendor.p({
          type: "eye",
        ...
        S: common_vendor.o(toggleAnswer)
      } : {
        T: common_vendor.p({
          type: "eye-slash",
          size: "16",
        ...
        U: common_vendor.o(toggleAnswer)
      }, {
        V: hasNextQuestion.value || practiceMode.value === "full" && canSwitchToNextChapter()
        ...
        X: common_vendor.o(nextQuestion)
      } : {
        Y: common_vendor.p({
          type: "checkmarkempty",
          size: "16",
        ...
        Z: common_vendor.o(finishExam)
      }) : {}, {
        t: currentQuestion.value,
        aa: common_vendor.p({
          type: "closeempty",
          size: "20",
          color: "#999"
        ...
        ac: common_vendor.t(answeredCount.value),
        ad: common_vendor.t(correctCount.value),
        ae: common_vendor.t(wrongCount.value),
        af: common_vendor.t(accuracy.value),
        ag: common_vendor.o(resetProgress),
        ah: common_vendor.o(closeStats),
        ai: common_vendor.sr(statsPopup, "7ab7c2ad-12", {
          "k": "statsPopup"
        }),
        aj: common_vendor.p({
          type: "center"
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-7ab7c2ad"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pkg-exam/pages/exam/exam.js.map

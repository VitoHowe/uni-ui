  ...
  setup(__props) {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:363", "获取科目失败:", error);
    ...
    const fetchPracticeSummary = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:400", "获取练习统计失败:", error);
        resetPracticeStats();
      }
    };
    ...
    common_vendor.onShow(() => {
      common_vendor.index.__f__("log", "at pkg-exam/pages/exam-list/exam-list.vue:500", "📱 题库列表页面显示，刷新数据...");
      initPage();
    });
    const fetchBankList = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pkg-exam/pages/exam-list/exam-list.vue:548", "✅ 题库列表加载完成，已使用study_progress优化接口调用");
      } catch (error) {
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:550", "获取题库列表失败:", error);
        common_vendor.index.showToast({
    ...
    const startExam = async (bank) => {
      ...
      if (!bank.chapters || bank.chapters.length === 0) {
        try {
          ...
          common_vendor.index.hideLoading();
          common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:660", `获取题库${bank.id}章节失败:`, error);
          common_vendor.index.showToast({
            title: "加载章节失败",
            icon: "none"
          });
    ...
    const startChapterPractice = async () => {
      ...
      if (bank.chapterProgress === null) {
        try {
          ...
          common_vendor.index.hideLoading();
          common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:687", `获取题库${bank.id}章节进度失败:`, error);
...
是否继续？`,
        ...
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:730", `获取整卷练习进度失败:`, error);
      }
      navigateToExam("full", startChapterId, startQuestionNumber);
    };
...
是否继续？`,
    ...
    const navigateToExam = (mode, chapterId, questionNumber = 1) => {
      ...
      common_vendor.index.navigateTo({
        url: `/pkg-exam/pages/exam/exam?bankId=${bank.id}&mode=${mode}&chapterId=${chapterId}&questionNumber=${questionNumber}${subjectParam}`
      });
    };
...
重置后将从第一题重新开始，此操作不可恢复。`,
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              ...
              common_vendor.index.__f__("log", "at pkg-exam/pages/exam-list/exam-list.vue:845", `🔄 题库 ${bank.bank_name} 学习进度已重置`);
            } catch (error) {
              common_vendor.index.hideLoading();
              common_vendor.index.__f__("error", "at pkg-exam/pages/exam-list/exam-list.vue:848", "重置进度失败:", error);
              common_vendor.index.showToast({
    ...
    const goToUpload = () => {
      common_vendor.index.navigateTo({
        url: "/pkg-tools/pages/upload/upload"
      });
    };
    return (_ctx, _cache) => {
      var _a, _b;
      return common_vendor.e({
        ...
        m: common_vendor.t(practiceStats.value.answered_count),
        n: common_vendor.t(practiceStats.value.accuracy),
        o: common_vendor.t(practiceStats.value.wrong_count),
        p: common_vendor.t(totalBanks.value),
        q: common_vendor.t(totalQuestions.value),
        r: common_vendor.t(todayAdded.value),
        s: loading.value
      }, loading.value ? {
        t: common_vendor.p({
          type: "spinner-cycle",
        ...
        y: common_vendor.o(goToUpload)
      } : {
        z: common_vendor.f(filteredBankList.value, (bank, k0, i0) => {
          return common_vendor.e({
            a: "ab283ccc-7-" + i0,
        ...
        H: common_vendor.o(applyFilter),
        I: common_vendor.sr(filterPopup, "ab283ccc-12", {
          "k": "filterPopup"
        }),
        J: common_vendor.p({
          type: "bottom"
        ...
        L: common_vendor.o(shareBank),
        M: selectedBank.value && selectedBank.value.completed_count > 0
        ...
        O: common_vendor.o(resetBankProgress)
      } : {}, {
        P: common_vendor.o(closeActionPopup),
        Q: common_vendor.sr(actionPopup, "ab283ccc-14", {
          "k": "actionPopup"
        }),
        R: common_vendor.p({
          type: "bottom"
        ...
        T: common_vendor.o(($event) => modePopup.value.close()),
        U: common_vendor.p({
          type: "list",
          size: "40",
          color: "#667eea"
        }),
        V: common_vendor.o(startChapterPractice),
        W: common_vendor.p({
          type: "paperplane",
          size: "40",
          color: "#f5576c"
        }),
        X: common_vendor.o(startFullPractice),
        Y: common_vendor.sr(modePopup, "ab283ccc-17", {
          "k": "modePopup"
        }),
        Z: common_vendor.p({
          type: "center"
        ...
        ab: common_vendor.o(($event) => chapterSelectPopup.value.close()),
        ac: common_vendor.f((_b = selectedBank.value) == null ? void 0 : _b.chapters, (chapter, k0, i0) => {
          return {
        ...
        ae: common_vendor.sr(chapterSelectPopup, "ab283ccc-21", {
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-ab283ccc"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pkg-exam/pages/exam-list/exam-list.js.map

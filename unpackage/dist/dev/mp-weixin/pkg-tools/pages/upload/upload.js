...
const pkgTools_stores_upload = require("../../stores/upload.js");
...
const _sfc_main = {
  __name: "upload",
  setup(__props) {
    const uploadStore = pkgTools_stores_upload.useUploadStore();
    ...
    const onFileSelect = (e) => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:218", "选择文件:", e);
    ...
    const onUploadProgress = (e) => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:230", "上传进度:", e);
    };
    const onUploadSuccess = (e) => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:236", "上传成功:", e);
    };
    const onUploadFail = (e) => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:243", "上传失败:", e);
    };
    const clearFiles = () => {
      if (filesList.value.length === 0)
        return;
...
状态：${record.status}${record.errorMessage ? "\n错误信息：" + record.errorMessage : ""}${record.recordCount ? "\n解析记录数：" + record.recordCount : ""}`,
    ...
    const checkUploadPermission = () => {
      ...
      if (!permissionResult.available) {
        common_vendor.index.showModal({
          ...
          success: (res) => {
            if (res.confirm) {
              utils_auth.RouteGuard.saveReturnPath("/pkg-tools/pages/upload/upload");
    ...
    const handleStartUpload = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pkg-tools/pages/upload/upload.vue:337", "上传失败:", error);
        common_vendor.index.showToast({
    ...
    const onTabChange = (index) => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:347", "切换到tab:", index);
    };
    common_vendor.onMounted(() => {
      common_vendor.index.__f__("log", "at pkg-tools/pages/upload/upload.vue:352", "文件上传页面加载");
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        e: common_vendor.p({
          ...
          ["show-preview"]: true,
          ["current-path"]: "/pkg-tools/pages/upload/upload"
        })
      } : common_vendor.e({
        ...
        m: common_vendor.sr("filePickerRef", "c808b687-7,c808b687-6"),
        ...
        K: common_vendor.f(uploadHistory.value, (record, index, i0) => {
          return {
            a: "c808b687-15-" + i0 + "," + ("c808b687-14-" + i0),
            b: common_vendor.p({
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-c808b687"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pkg-tools/pages/upload/upload.js.map
